package net.digitallogic.PropertyExpander;
...
public class Utils {
	...
	public static List<Sort.Order> processSortBy(String str) {
		return Arrays.stream(splitOnComma.split(str.trim()))
			// transform property to cameCase if it is in snake_case.
			.map(Utils::toCamel)
			.map(property -> {
				if (property.charAt(0) == '-')
					return new Sort.Order(Sort.Direction.DESC, property.substring(1));
				else
